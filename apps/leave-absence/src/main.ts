...
import { LeaveAbsenceModule } from './leave-absence.module';
import { ValidationPipe } from '@nestjs/common';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';

async function bootstrap() {
  const app = await NestFactory.create(LeaveAbsenceModule, {
    cors: {
      origin: '*',
  ...
  await app.listen(process.env.LEAVE_SERVICE_PORT ?? 3000);
}
bootstrap();
