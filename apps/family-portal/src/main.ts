...
import { FamilyPortalModule } from './family-portal.module';
import { ValidationPipe } from '@nestjs/common';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';

async function bootstrap() {
  const app = await NestFactory.create(FamilyPortalModule, {
    cors: {
      origin: '*',
  ...
  await app.listen(process.env.FAMILY_SERVICE_PORT || 7010);
}
bootstrap();
