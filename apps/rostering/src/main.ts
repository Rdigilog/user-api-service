...
import { RosteringModule } from './rostering.module';
import { ValidationPipe } from '@nestjs/common';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';

async function bootstrap() {
  const app = await NestFactory.create(RosteringModule, {
    cors: {
      origin: '*',
  ...
  await app.listen(process.env.ROSTERING_SERVICE_PORT || 7014);
}
bootstrap();
