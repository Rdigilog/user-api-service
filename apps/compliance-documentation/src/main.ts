...
import { ComplianceDocumentationModule } from './compliance-documentation.module';
import { ValidationPipe } from '@nestjs/common';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';

async function bootstrap() {
  const app = await NestFactory.create(ComplianceDocumentationModule, {
    cors: {
      origin: '*',
  ...
  await app.listen(process.env.COMPLIANCE_SERVICE_PORT || 7009);
}
bootstrap();
