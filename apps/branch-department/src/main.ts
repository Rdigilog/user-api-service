...
import { BranchDepartmentModule } from './branch-department.module';
import { ValidationPipe } from '@nestjs/common';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';

async function bootstrap() {
  const app = await NestFactory.create(BranchDepartmentModule, {
    cors: {
      origin: '*',
  ...
  await app.listen(process.env.BRANCH_SERVICE_PORT || 7006);
}
bootstrap();
