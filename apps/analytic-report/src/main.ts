...
import { AnalyticReportModule } from './analytic-report.module';
import { ValidationPipe } from '@nestjs/common';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';

async function bootstrap() {
  const app = await NestFactory.create(AnalyticReportModule, {
    cors: {
      origin: '*',
  ...
  await app.listen(process.env.ANALYTIC_SERVICE_PORT || 7002);
}
bootstrap();
