...
import { RecordnotesModule } from './recordnotes.module';
import { ValidationPipe } from '@nestjs/common';
import { DocumentBuilder, SwaggerModule } from '@nestjs/swagger';

async function bootstrap() {
  const app = await NestFactory.create(RecordnotesModule, {
    cors: {
      origin: '*',
  ...
  await app.listen(process.env.RECORDNOTE_SERVICE_PORT || 7013);
}
bootstrap();
