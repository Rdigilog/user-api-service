...
import { OtpService } from '@app/utils/services/otp.service';
import { Injectable } from '@nestjs/common';
import { Prisma } from '@prisma/client';
import { InviteEmailFields, SendMailDto } from '@app/mail/dtos/mail.dto';
import { mailSubjects, mailTemplates } from '@app/mail/enums/subjects.enum';
import {
  CompanyDetailsDTO,
  InitiateRegistrationDto,
  InviteUserDTO,
  PhoneNumberDTO,
} from '@app/model/onboarding/SignUp.dto';
...
import {
  ...
  InjectMailQueue,
  InjectSMSQueue,
} from '@app/queue/decorators/queue.decorator';
import { Queue } from 'bullmq';
import {
  EmployeeDto,
  UpdateProfileDto,
} from '@app/model/onboarding/profile.dto';
  ...
  async createAccount(payload: InitiateRegistrationDto) {
    try {
      const result = await this.$transaction(async (tx) => {
        ...
        return await this.sendOtp(result.email);
      });
      return result;
    } catch (e) {
    ...
    try {
      ...
      const invite = await this.invitation.create({
        data: {
          ...
          inviteLink: this.utilsService.lisaUnique(),
        },
        include: {
          invitedByUser: {
            select: {
